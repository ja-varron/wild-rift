...
const TOPICS = ["Agriculture", "Biology", "Chemistry", "Physics"]

const QuestionAnswerKey = ({ questionNumber }: { questionNumber: number }) => (
  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 border-2 border-[#E6E6E7] p-2">
    {/* Question number */}
    <ErrorBoundary fallback={<div>Error Loading Question Answer Key</div>}>
    ...
    {/* TODO: Add input for points */}
    <div className="flex flex-row items-center gap-x-2 items-center justify-center">
      <p className="text-sm">Points:</p>
      <Input type="number" className="flex max-w-16" defaultValue={1} />
    </div>
    
    {/* TODO: Dynamically load topics from the course */}
    <div className="flex flex-row items-center gap-x-2 items-center justify-center">
      ...
      <Select defaultValue="selected">
        ...
        <SelectContent>
          <SelectItem value="selected">Select Topic/Tag</SelectItem>
          {TOPICS.map((topic) => (
      ...
      <div className="flex flex-col gap-y-6 w-full">
        ...
        <div className="flex flex-col gap-5">
          {Array.from({ length: 100 }, (_, index) => (
            <QuestionAnswerKey key={index} questionNumber={index + 1} />
          ))}
        </div>
      </div>
    </div>
