...
const ExamAccordionList = ({ results }: ExamAccordionListProps) => {
  ...
  const pending = results.filter((r) => !r.attempted)
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <span>{attempted.length} taken</span>
          {pending.length > 0 && (
            <>
              <span>·</span>
      ...
      {results.length === 0 ? (
        ...
        <Accordion type="multiple" className="space-y-2">
          {results.map((r) => {
            const pct = r.attempted && r.totalItems > 0
                          ...
                          <TabsContent value="result" className="mt-4">
                            <div className="grid grid-cols-3 gap-4">
                              ...
                              <div className="rounded-lg bg-muted/50 p-3 text-center space-y-0.5">
                                ...
                                <p className="text-xl font-bold tabular-nums">{r.passingRate}%</p>
                              </div>
                            </div>
                          </TabsContent>
